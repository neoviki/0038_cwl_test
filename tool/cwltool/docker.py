            ...
            if "dockerPull" in docker_requirement:
                ...
                if os.path.exists(docker_requirement["dockerLoad"]):
                    ...
                    with open(docker_requirement["dockerLoad"], "rb") as dload:
                        ...
                        frameinfo = getframeinfo(currentframe())
                        viki_logger(frameinfo.filename, frameinfo.lineno, viki_cmd)
                        loadproc = viki_job_executer(cmd, 
                                 shell=None,
                                 close_fds=None,stdin=dload, stdout=sys.stderr, env=None, cwd=None)
                else:
                    viki_cmd = cmd
                    frameinfo = getframeinfo(currentframe())
                    viki_logger(frameinfo.filename, frameinfo.lineno, viki_cmd)
                    loadproc = viki_job_executer(cmd, shell=None, close_fds=None, stdin=subprocess.PIPE,
                                                stdout=sys.stderr, env=None, cwd=None)
