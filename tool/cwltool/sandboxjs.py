...
def check_js_threshold_version(working_alias):
    ...
    viki_cmd = [working_alias, "-v"]
    frameinfo = getframeinfo(currentframe())
    viki_logger(frameinfo.filename, frameinfo.lineno, viki_cmd)
    current_version_str = subprocess.check_output([working_alias, "-v"]).decode('utf-8')
...
def new_js_proc(js_text, force_docker_pull=False):
    ...
    for n in trynodes:
        try:
            viki_cmd = [n, "--eval", "process.stdout.write('t')"]
            frameinfo = getframeinfo(currentframe())
            viki_logger(frameinfo.filename, frameinfo.lineno, viki_cmd)
            if subprocess.check_output([n, "--eval", "process.stdout.write('t')"]).decode('utf-8') != "t":
                continue
            else:
                viki_cmd = [n, "--eval", js_text]
                frameinfo = getframeinfo(currentframe())
                viki_logger(frameinfo.filename, frameinfo.lineno, viki_cmd)
                nodejs = viki_job_executer([n, "--eval", js_text],
                                          shell=None,
                                          close_fds=None,
                                          stdin=subprocess.PIPE,
                                          stdout=subprocess.PIPE,
                                          stderr=subprocess.PIPE, env=None, cwd=None)
    ...
    if nodejs is None or nodejs is not None and required_node_version is False:
        try:
            ...
            if not have_node_slim:
                viki_cmd = ["docker", "images", "-q", nodeimg]
                frameinfo = getframeinfo(currentframe())
                viki_logger(frameinfo.filename, frameinfo.lineno, viki_cmd)
                ...
                if (len(dockerimgs.split("\n")) <= 1) or force_docker_pull:
                    ...
                    viki_cmd = ["docker", "pull", nodeimg]
                    frameinfo = getframeinfo(currentframe())
                    viki_logger(frameinfo.filename, frameinfo.lineno, viki_cmd)
                    nodejsimg = subprocess.check_output(["docker", "pull", nodeimg]).decode('utf-8')
                    _logger.info("Pulled Docker image %s %s", nodeimg, nodejsimg)
                have_node_slim = True
            viki_cmd = ["docker", "run","--attach=STDIN", "--attach=STDOUT", "--attach=STDERR","--sig-proxy=true", "--interactive",
                                       "--rm", nodeimg, "node", "--eval", js_text]
            frameinfo = getframeinfo(currentframe())
            viki_logger(frameinfo.filename, frameinfo.lineno, viki_cmd)

            nodejs = viki_job_executer(["docker", "run",
                                       "--attach=STDIN", "--attach=STDOUT", "--attach=STDERR",
                                       "--sig-proxy=true", "--interactive",
                                       "--rm", nodeimg, "node", "--eval", js_text],
                                       shell=None,
                                       close_fds=None,
                                      stdin=subprocess.PIPE, stdout=subprocess.PIPE, stderr=subprocess.PIPE, env=None,cwd=None)
            docker = True
        except OSError as e:
