...
from .viki_log import viki_logger
from .viki_log import viki_job_executer
viki_ctr=0
...
class JobBase(with_metaclass(ABCMeta, HasReqsHints)):
    ...
    def _execute_viki(self,
                 runtime,  # type: List[Text]
                 env,  # type: MutableMapping[Text, Text]
                 runtimeContext  # type: RuntimeContext
                 ):  # type: (...) -> None
        print "viki JobBase -> _execute()"
        ...
        try:
            ...
            # Viki Comment, Convert command in list format to string(Text)
    ...
    def _execute(self,
        ...
        try:
            ...
            if self.stdout:
                ...
                stdout_path = absout
            print "-----------------------------------------------------"
            print "viki_runtime = ", runtime
            print "-----------------------------------------------------"
            print "viki_command_line = ", self.command_line
            print "-----------------------------------------------------"
            #Viki Comment, Convert command in list format to string(Text)
            commands = [Text(x) for x in (runtime + self.command_line)]
            print "-----------------------------------------------------"
            ...
            if self.generatefiles["listing"]:
                #viki commented assertion 
                #assert self.generatemapper is not None
                print("viki assert [ %s : %d ]")%(getframeinfo(currentframe()).filename, getframeinfo(currentframe()).lineno)
        ...
        if processStatus != "success":
            _logger.warning(u"[job %s] viki job completed %s", self.name, processStatus)
        else:
            _logger.info(u"[job %s] viki job completed %s", self.name, processStatus)
...
class CommandLineJob(JobBase):
    ...
    def run(self,
        ...
        print "viki_flow CommandLineJob(JobBase) -> run()"
        ...
        print "viki_flow calling *._execute()"
        self._execute_viki([], env, runtimeContext)
...
class ContainerCommandLineJob(with_metaclass(ABCMeta, JobBase)):
    ...
    def run(self, runtimeContext):
        print "viki_flow ContainerCommandLineJob(JobBase) -> run()"
        ...
        print runtime
        self._execute_viki(runtime, env, runtimeContext)
...
def _job_popen(
    ...
    print "viki_flow JobBase -> _job_popen()"

    _logger.warning("viki job_popen called")
    if not job_script_contents and not FORCE_SHELLED_POPEN:
        ...
        frameinfo = getframeinfo(currentframe())
        viki_logger(frameinfo.filename, frameinfo.lineno, commands)
        sproc = viki_job_executer(commands,
                                 shell=False,
        ...
        try:
            ...
            print("viki [ %s : %d ]")%(getframeinfo(currentframe()).filename, getframeinfo(currentframe()).lineno)
            viki_command = ["bash", job_script.encode("utf-8")]
            frameinfo = getframeinfo(currentframe())
            viki_logger(frameinfo.filename, frameinfo.lineno, viki_command)
            print("viki [ %s : %d ]")%(getframeinfo(currentframe()).filename, getframeinfo(currentframe()).lineno)

            sproc = viki_job_executer(
